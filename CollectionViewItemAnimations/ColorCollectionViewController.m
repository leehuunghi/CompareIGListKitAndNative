...
- (void)deleteSectionAtIndex:(NSUInteger)index
{
    ...
    [self.sectionedColorNames removeObjectAtIndex:index+1];
    
    [self.sectionedColorNames insertObject:[NSMutableArray array] atIndex:index+3];

    // Batch this so the other sections will be updated on completion
    [self.collectionView performBatchUpdates:^{
//        [self.collectionView deleteSections:[NSIndexSet indexSetWithIndex:index]];
        NSMutableIndexSet *set = [NSMutableIndexSet indexSetWithIndex:index];
        [set addIndex:index + 1];
        [self.collectionView deleteSections:set];
        
        [self.collectionView insertSections:[NSIndexSet indexSetWithIndex:index+3]];
